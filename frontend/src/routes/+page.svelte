	...
	{:else}
		...
		<section class="bg-white dark:bg-slate-900 rounded-[2rem] p-10 border border-slate-200/60 dark:border-slate-800 shadow-sm flex flex-col gap-8 group/section transition-all duration-500">
			...
			<div class="flex flex-wrap gap-4">
				{#each tables.slice(0, 6) as t}
					<a 
						href={`/table/${t}`}
						class="px-6 py-4 bg-slate-50/40 dark:bg-slate-950/40 rounded-2xl border border-slate-100 dark:border-slate-800/60 hover:border-emerald-300 dark:hover:border-emerald-800 hover:bg-white dark:hover:bg-slate-800 transition-all duration-500 flex items-center gap-4 group"
					>
						<span class="w-2 h-2 rounded-full bg-emerald-500 shadow-lg shadow-emerald-500/50 group-hover:animate-ping"></span>
						<span class="font-black text-slate-800 dark:text-slate-200 uppercase tracking-tight text-sm italic group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors uppercase italic">{t}</span>
					</a>
				{:else}
					<div class="py-4 w-full flex justify-center italic text-slate-300 dark:text-slate-700 uppercase font-black tracking-widest text-lg opacity-30">Void Schema</div>
				{/each}
			</div>
		</section>
	{/if}
